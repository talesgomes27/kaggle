...
Now that we hav the data loaded we can use glimpse to see it.
...
## Data Manipulation
  ...
  Wen can see that R imported all the data as numeric, but as we know that some variable are categorical variable. Than to facilitate the manipulation wen can store in two variables that information.
...
First thing wen can do is to verify if that is some NAs in the data.
...
Now we can see how tha dataset is with glimpse.
...
first we can calculate the mean and standart variable of the data to see the need to scale the data.
...
Now let's shoe the data distribution 
...
As we can see some variables are on different scales and so we will perform the normalization of the data.
...
one last verification if we don't introduced some NAs in the data
...
Before starting to model the data, we have to verify that it is balanced.
...
Now our data is balanced, propor maked as numerical and categorical and normalized. So we can train our first model. As a base model we will use the logistic model with all the variable of the data set.

#### Split the data

First wen will separate the data in train and test. For tha we used dplyr
...
In this dataset we have a big problem, the data is small and for that reason we decide to procede a [k-fold cross-validation](https://en.wikipedia.org/wiki/Cross-validation_(statistics)) of our model and on top of that we will proced a [stepwise modeling](https://www.statology.org/stepwise-regression-r/).
...
Now we will train out model for each folder and for last traing a model with the whole data set.
...
As we can see we produce a model with 89% accuracy a good improvement from the base model. Now lets plot the confusion matrix.
